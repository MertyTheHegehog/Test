public class Squirrel {
    ...
    public int maxNuts() {
        ...
        if (len > 1) {
            ...
            if (nuts[i] > 0) maxOfNuts += nuts[i];
            //Проверяем, можно ли подобрать орешки со второго до последнего-предпоследнего
            for (; i < len - 2; i++) {
                if (nuts[i + 1] < 0 && nuts[i + 1] <= nuts[i + 2]) i++;//перепрыгиваем через пень
                maxOfNuts += nuts[i + 1];
                if (maxOfNuts < 0) maxOfNuts = 0;
            }
            // Смотрим, стоит ли прыгать на последний пень
            if (i == len - 2 && nuts[i + 1] > 0) maxOfNuts += nuts[i + 1];
            return maxOfNuts;
        } else if (nuts[0] >= 0) return nuts[0];
        else return 0;
    }
    //*/
}
